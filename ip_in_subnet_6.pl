...
sub ip_compress_address {
    ...
    unless ($ip_version) {
        die ("Cannot determine IP version for $ip");
    }
...
sub ip_bintoip {
    ...
    if ($len < length($binip)) {
        die ("Invalid IP length for binary IP $binip\n");
    }
...
unless (@ARGV == 2)
{
     print "\nUsage: $0 <IPv6> <network/CIDR>\n";
     exit;
}

my $ip = $ARGV[0];
my $network = lc ( $ARGV[1] );

if ( ! ( &valid_ipv6_host ( $ip ) ) )
{
        print "[*] $0: $ip does not seem to be a valid IPv6 host\n";
        exit(1);
}

#####  main  #####
$ip            = lc (&ip_expand_address   ( $ip, 6 ) );
my $compressed =     &ip_compress_address ( $ip, 6 );

if ( &in_subnet_ipv6 ($network, "$ip/128") )
{
        print "\n[*] $0: SUCCESS => $ip (compressed $compressed) found in $network\n\n";
} else {
        print "\n[*] $0: FAILED  => $ip (compressed $compressed) NOT found in $network\n\n";
